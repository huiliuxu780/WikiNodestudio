...
test.describe("Source / Raw Material / Parsed Document acceptance", () => {
  ...
  test("Sources page exposes Source import actions without explainer cards", async ({ page }) => {
    ...
    await expect(page.getByRole("columnheader", { name: "Knowledge Base" })).toBeVisible()
    await expect(page.getByRole("columnheader", { name: "接入模式" })).toBeVisible()
    await expect(page.getByRole("columnheader", { name: "凭据状态" })).toBeVisible()
    await expect(page.getByRole("columnheader", { name: "连接状态" })).toBeVisible()
    await expect(page.getByRole("columnheader", { name: "最近检查" })).toBeVisible()
    await expect(page.getByRole("columnheader", { name: "处理记录" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "CC After-sales KB" })).toHaveCount(2)
    await expect(page.getByRole("cell", { name: "定时同步" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "已配置" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "缺少凭据" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "连接可用" })).toHaveCount(2)
    await expect(page.getByRole("cell", { name: "2026-06-20 10:25" })).toBeVisible()
    ...
    await expect(page.getByRole("link", { name: "查看 Source" }).first()).toHaveAttribute("href", "/sources/src-feishu-cc")
    await expect(page.getByText("生成的 WikiNode", { exact: true })).toHaveCount(0)
    await expect(page.getByText("当前来源暂无关联 WikiNode。", { exact: true })).toHaveCount(0)
  ...
  test("Source detail points to related raw material and processing status", async ({ page }) => {
    ...
    await expect(page.getByText("连接可用", { exact: true })).toBeVisible()
    await expect(page.getByText("已配置", { exact: true })).toBeVisible()
    await expect(page.getByRole("tab", { name: "接入配置" })).toBeVisible()
    await expect(page.getByRole("tab", { name: "处理记录" })).toBeVisible()
    await page.getByRole("tab", { name: "接入配置" }).click()
    await expect(page.getByText("连接器配置", { exact: true })).toBeVisible()
    await expect(page.getByText("凭据与连接状态", { exact: true })).toBeVisible()
    await expect(page.getByRole("cell", { name: "接入模式" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "定时同步" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "同步策略" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "每日" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "默认 Parser Profile" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "feishu_article_v1" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "凭据 Profile" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "feishu-after-sales-space" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "凭据范围" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "售后知识空间只读" })).toBeVisible()
    await expect(page.getByRole("cell", { name: "凭据负责人" })).toBeVisible()
    await expect(page.getByRole("row", { name: /凭据负责人/ })).toContainText("售后运营")
    await expect(page.locator("main").last()).not.toContainText(/token|password|secret|private key|Authorization|Bearer/i)
    await page.getByRole("tab", { name: "处理记录" }).click()
    await expect(page.getByRole("columnheader", { name: "操作类型" })).toBeVisible()
    await expect(page.getByRole("columnheader", { name: "Raw Material" })).toBeVisible()
    await expect(page.getByText("来源同步", { exact: true })).toBeVisible()
    await expect(page.getByText("连接检查", { exact: true })).toBeVisible()
    await expect(page.getByText("解析 Raw Material", { exact: true })).toBeVisible()
