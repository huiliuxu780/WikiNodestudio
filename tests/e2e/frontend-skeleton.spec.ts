...
test.describe("Frontend skeleton IA", () => {
  ...
  test("Sources and Raw Materials show evidence-chain work surfaces without boundary copy", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "导入文件" }).first()).toBeVisible()
    ...
    await expect(page.getByText("Source 是原始知识的来源。")).toHaveCount(0)
    await expect(page.getByText("记录来源类型、负责人、同步状态和最近更新时间。")).toHaveCount(0)
    await expect(page.getByText("保留进入标准化处理前的原始材料证据。")).toHaveCount(0)
    await expect(page.locator("main").last()).not.toContainText(forbiddenProductTerms)
    ...
    await expect(page.locator("#source-import")).toHaveCount(0)
    await expect(page.getByText("来源处理状态")).toHaveCount(0)
    await expect(page.getByText("按 Source、Raw Material、Parsed Document 和 WikiNode 追踪来源处理进度。")).toHaveCount(0)
    await expect(page.locator("main").last()).not.toContainText(forbiddenProductTerms)
    await expect(page.locator("main").last()).not.toContainText(forbiddenBoundaryCopy)
